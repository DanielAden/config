...
-- I borrowed pretty much all of the following from here:
-- ray-x/go.nvim lua/snips/go.lua
...
local function transform(text, info)
	...
	elseif text == "error" then
		...
		info.index = info.index + 1
		return snips.go_err_snippet({ { info.err_name } }, nil, nil, { index = info.index })
...
snips.make_return_nodes = function(args)
	local info = { index = 0, err_name = args[1][1] }

	return ls.sn(nil, return_value_nodes(info))
end
snips.make_default_return_nodes = function()
	local info = { index = 0, err_name = "nil" }

	return ls.sn(nil, return_value_nodes(info))
end

snips.fill_return = function()
	local info = { index = 0, err_name = "nil" }
	return ls.sn(nil, return_value_nodes(info))
end

snips.last_lua_module_section = function(args)
...
function snips.is_in_test_file()
	local filename = vim.fn.expand("%:p")
	return vim.endswith(filename, "_test.go")
end

function snips.is_in_test_function()
	return snips.is_in_test_file() and snips.is_in_function()
end

snips.create_t_run = function(args)
	return ls.sn(1, {
		ls.c(1, {
			ls.t({ "" }),
			ls.sn(
...
snips.mirror_t_run_funcs = function(args)
	local strs = {}
...
return {
	ls.s(
		{ trig = "ferrif", name = "func + err if", dscr = "Call a function and check the error" },
		fmt(
			[[
        {err1} := {func}({args})
        if {err2} != nil {{
	...
	ls.s(
		{ trig = "ferrvif", name = "func + val + err if", dscr = "Call a function and check the error" },
		fmt(
			[[
				{val}, {err1} := {func}({args})
        if {err2} != nil {{
